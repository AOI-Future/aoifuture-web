...
- 例外的に個人 repo のまま製品化途上のものもある（現状 `0xshugo/aoi-tap`）。その場合は **製品化の意思が固まった時点で Org へ移管**するのが正。移管前でも紹介ページは先行して作ってよい（コードの置き場所とページの有無は厳密には独立）。
...
| 対象 | 置き場所 | 理由 |
|---|---|---|
| アプリ本体のコード | **各アプリの独立リポ**（例 `0xshugo/aoi-tap`, `AOI-Future/aoi-harbor`） | Web サイトにアプリコードを統合しない |
