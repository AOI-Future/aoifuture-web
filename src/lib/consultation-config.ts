...
export function getConsultationConfig(env: Record<string, string | undefined> = process.env) {
  ...
  return {
    ...
    fallbackUrl: env.PUBLIC_CONSULTATION_FALLBACK_URL || env.CONSULTATION_FALLBACK_URL || 'https://aoifuture.notion.site/6a828f1f8371416187ab544742734fcd?pvs=105',
    ...
    distributedLimitUrl: env.CONSULTATION_RATE_LIMIT_REST_URL || '',
    distributedLimitToken: env.CONSULTATION_RATE_LIMIT_REST_TOKEN || '',
    distributedLimitHashSecret: env.CONSULTATION_RATE_LIMIT_HASH_SECRET || '',
    distributedLimitNamespace: env.CONSULTATION_RATE_LIMIT_NAMESPACE || env.VERCEL_ENV || 'development',
    requireDistributedLimit: truthy(env.CONSULTATION_REQUIRE_DISTRIBUTED_LIMIT),
  };
}
